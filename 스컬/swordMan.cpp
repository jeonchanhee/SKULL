...
HRESULT swordMan::init(int x, int y)
{
	_img = IMAGEMANAGER->findImage("ÀÎ°£°Ë");
...
void swordMan::attack()
{
	...
	if (_attCount % 20 == 0)
	{
		...
		if (_state == E_LEFT || _state == E_LEFT_STOP)
		{
			...
			*_attRc = RectMakeCenter(_hitRc.left - 5, _y, 50, 50);
		}
		else if (_state == E_RIGHT || _state == E_RIGHT_STOP)
		{
			...
			*_attRc = RectMakeCenter(_hitRc.right + 5, _y, 50, 50);
		}
	}
}
